...
def get_custom_metadata(info, audio):
    if "pixabay" in info['path']:
        # print(f'Getting dir path{info["path"].split("/")[2:]}')
        seconds_start = info['seconds_start']
        seconds_end = seconds_start + info['seconds_total']
        # print(f'Second start for this track: {seconds_start}, seconds end: {seconds_end}')
        info_path = Path(info["path"])
        dir_path = info_path.parent
        metadata_file = dir_path / "metadata_tags.json"
        # print(f'Metadata file path: {metadata_file}')
        if not os.path.exists(metadata_file):
            raise FileNotFoundError(f"Metadata file not found in {metadata_file}")
        
        with open(metadata_file, 'r') as f:
            dataset_metadata = json.load(f)

        track_identifier = os.path.join(*info['path'].split('/')[-1:]).replace('\\', '/')
        # print(f'track_identifier: {track_identifier}')
        if track_identifier not in dataset_metadata:
            raise KeyError(f"No metadata found for {track_identifier}")
        
        track_metadata = dataset_metadata[track_identifier]
        chunks = track_metadata.get('chunks', [])
        ...
        for chunk in chunks:
            ...
            if seconds_start < chunk_end and seconds_end > chunk_start:
                tags_words = chunk['tags'].split()
                unique_words = [word for word in tags_words if word not in seen_words]
                seen_words.update(unique_words)
                if combined_tags:
                    combined_tags += " "
                combined_tags += " ".join(unique_words)
        bpm = track_metadata.get("bpm")
        # print(f'getting bpm {bpm}')
        if bpm is not None:
            combined_tags += f", {bpm} BPM"
                
        custom_metadata = {
            "prompt": combined_tags
        }
    elif "bbc_sound" in info['path']:
        metadata_file = '/scratch/lz2807/bbc_sound/metadata_tags.json'
        if not os.path.exists(metadata_file):
            raise MetadataNotFoundError(f"Metadata file not found in {metadata_file}")
        with open(metadata_file, 'r') as f:
            dataset_metadata = json.load(f)
        ...
        prompt = metadata
        custom_metadata = {"prompt": prompt}
    elif "slakh2100_flac_redux" in info['path']:
        metadata_file = '/scratch/lz2807/slakh2100_flac_redux/metadata_tags.json'
        if not os.path.exists(metadata_file):
            raise MetadataNotFoundError(f"Metadata file not found in {metadata_file}")
        with open(metadata_file, 'r') as f:
            dataset_metadata = json.load(f)
        ...
        if "slakh2100_1st_iter" in info['path']:
            metadata_file = '/scratch/lz2807/slakh2100_1st_iter/metadata_tags1.json'
        elif "ballroom" in info['path']:
            metadata_file = '/scratch/lz2807/ballroom/audio/metadata_tags1.json'
        elif "guitarset" in info['path']:
            metadata_file = '/scratch/lz2807/guitarset/metadata_tags1.json'
        elif "brid" in info['path']:
            metadata_file = '/scratch/lz2807/metadata_tags1.json'
        elif "candombe" in info['path']:
            metadata_file = '/scratch/lz2807/candombe/metadata_tags1.json'
        elif "cuidado" in info['path']:
            metadata_file = '/scratch/lz2807/cuidado/metadata_tags1.json'
        elif "gtzan_genres" in info['path']:
            metadata_file = '/scratch/lz2807/gtzan_genres/metadata_tags1.json'
        elif "hainsworth" in info['path']:
            metadata_file = '/scratch/lz2807/hainsworth/metadata_tags1.json'
        elif "maestro" in info['path']:
            metadata_file = '/scratch/lz2807/maestro/metadata_tags1.json'
    ...
    print(f'getting custom_metadata {custom_metadata}')
    return custom_metadata
...
class SampleDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            audio, t_start, t_end, seconds_start, seconds_total, padding_mask = self.pad_crop(audio, sr)
